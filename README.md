...
CRISP utilizes the GC×GC-FORMS contour images from ChromaTOF, which is mainly based on Windows OS. The CIRSP has been well-tested to run on Windows 10 OS (64 bit) but all major function are compatible with Ubuntu 18.01 with all Python environment installed, based on requirements_(cpu|gpu).txt file. However, as CRISP depends on multiple external libraries, few manual adjustments may be required.
